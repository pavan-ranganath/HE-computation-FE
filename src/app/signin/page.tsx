  ...
  const onSubmit = async (data: { email: string }) => {
    try {
      ...
      await signInWithWebauthn(
        ...
        callbackUrl || '/home',
        openConfirmDialog,
      ); // Call the signInWithWebauthn function to initiate WebAuthn authentication.
    } catch (err) {
      console.error(err);
    } finally {
      setLoadingMessage('');
...
async function signInWithWebauthn(
  ...
  if (optionsResponse.status !== 200) {
    ...
    if (error) {
      ...
      openConfirmDialog({
        title: 'Error',
        content: 'Could not get authentication options from the server',
        hideCancelButton: true,
      });
  ...
  if (!opt.allowCredentials || opt.allowCredentials.length === 0) {
    ...
    openConfirmDialog({
      title: 'Error',
      content: 'There is no registered credential.',
      hideCancelButton: true,
    });
