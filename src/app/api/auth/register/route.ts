...
export async function GET(req: NextRequest) {
  ...
  if (session) {
    ...
    if (!options) {
      return NextResponse.json(
        { error: 'Error generating options' },
        { status: 400 },
      );
    }
    // Exclude previously registered credentials from the registration options
    options.excludeCredentials = credentials?.passkeyInfo.map(
      (c: {
        credentialId: any;
        credential: { response: { transports: any } };
      }) => ({
        id: c.credentialId,
        type: 'public-key',
  ...
  const { fullName, dob, email, mobile } = Object.fromEntries(
  ...
  setSession(response, {
    name: PASSKEY_SESSION_NAME,
    value: { user: { email, fullName, dob, mobile } },
  });
  return response;
}
