...
import type { passkeyObj } from 'types/nextauth';
 ...
 */
export async function saveCredentials(cred: User) {
  // Saving the user's credentials in the "credentials" collection of the database
  await mongoose.connection.db?.collection<User>(AUTH_TABLE).insertOne({
    id: cred.userID,
...
export async function updateCredentials(cred: passkeyObj, userID: string) {
  ...
  await mongoose.connection.db?.collection<User>(AUTH_TABLE).updateOne(
    {
      userID,
    },
    {
...
export function handleRegistrationError(
  ...
  if (error.name === 'NotAllowedError') {
    if (error.message.includes('Operation failed')) {
      setAlertBarProps({
        message: 'The selected authenticator is already registered.',
        severity: 'error',
      });
      ...
      setAlertBarProps({
        message:
          'The operation either timed out or was not allowed or authenticator is already registered',
        severity: 'error',
      });
      // toast.error(
      //   'The operation either timed out or was not allowed or authenticator is already registered',
...
export const authOptions: AuthOptions = {
            ...
            await verifyAuthenticationResponse({
              ...
              authenticator: {
                credentialPublicKey: passkeyInfo.registrationInfo
                  .registrationInfo?.credentialPublicKey.buffer as Uint8Array,
  ...
  callbacks: {
    jwt: async ({ token, user }) => {
      if (user) {
        token.is2FAVerified = !user.currentChallenge;
      }
      return token;
    },
